...
// ==/UserScript==
;
(()=>{var h=unsafeWindow,$=(e,...t)=>h.console.log(`%c[exlg] ${e}`,"color: #0e90d2;",...t),I=(e,...t)=>h.console.warn(`%c[exlg] ${e}`,"color: #0e90d2;",...t),E=(e,...t)=>{throw h.console.error(`%c[exlg] ${e}`,"color: #0e90d2;",...t),Error(t.join(" "))},b=null,N=null;location.host==="www.luogu.com.cn"&&!/blog/g.test(location.href)&&(/(\?|&)_contentOnly($|=)/g.test(location.search)&&E("Content-Only pages."),h._feInjection.code!==200&&E("Luogu failed to load. Exlg stops loading."),b=h._feInjection.currentData,N=h._feInjection.currentUser);var o=jQuery.extend({double:(e,t,r)=>[e(t),e(r)]}),q=new filterXSS.FilterXSS({onTagAttr:(e,t,r)=>{if(t==="style")return`${t}="${r}"`}});Date.prototype.format=function(e,t){t=t?"UTC":"";let r={"y+":this[`get${t}FullYear`](),"m+":this[`get${t}Month`]()+1,"d+":this[`get${t}Date`](),"H+":this[`get${t}Hours`](),"M+":this[`get${t}Minutes`](),"S+":this[`get${t}Seconds`](),"s+":this[`get${t}Milliseconds`]()};for(let n in r)RegExp(`(${n})`).test(e)&&(e=e.replace(RegExp.$1,("000"+r[n]).substr(r[n].toString().length+3-RegExp.$1.length)));return e};String.prototype.toInitialCase=function(){return this[0].toUpperCase()+this.slice(1)};Array.prototype.lastElem=function(){return this[this.length-1]};var D=(e,t)=>{if(!e)return"<<";let r=(u,_)=>u===_?"==":u<_?"<<":">>",n=["pre","alpha","beta"],[[i,l],[s,a]]=[e,t].map(u=>u.split(" "));if(i===s)return r(...[l,a].map(u=>u?n.findIndex(_=>_===u):1/0));let[d,g]=[i,s].map(u=>u.split("."));for(let[u,_]of d.entries())if(_!==g[u])return r(+_||0,+g[u]||0)},K=(e=1)=>~~(new Date().getTime()/e),P=e=>new Promise((t,r)=>o.get(e+(e.includes("?")?"&":"?")+"_contentOnly=1",n=>{n.code!==200&&r(`Requesting failure code: ${t.code}.`),t(n)})),B=h.show_alert?(e,t="exlg 提醒您")=>h.show_alert(t,e):(e,t="exlg 提醒您")=>h.alert(t+`
`+e),X=null,U=(e,t)=>o.ajax({url:e,data:t,headers:{"x-csrf-token":X===null?X=o("meta[name=csrf-token]").attr("content"):X,"content-type":"application/json"},method:"post"}),M=({url:e,onload:t,onerror:r=n=>E(n)})=>GM_xmlhttpRequest({url:e,method:"GET",onload:t,onerror:r}),R=(e,t)=>{let r=new URLSearchParams;for(let i in e)r.set(i,e[i]);let n=o(`
    ...
    `,n=s=>s<=3?"":r.replace("%",s<=5?"#5eb95e":s<=7?"#3498db":"#f1c40f"),i=h.loadFeed;h.loadFeed=function(){h.feedMode==="all-exlg"?(M({url:e.source==="o2"?"https://service-ig5px5gh-1305163805.sh.apigw.tencentcs.com/release/APIGWHtmlDemo-1615602121":`https://bens.rotriw.com/api/list/proxy?page=${h.feedPage}`,onload:s=>{JSON.parse(s.response).forEach(d=>o(`
            ...
            `).appendTo(o("ul#feed")).find("a[name=feed-reply]").on("click",()=>{scrollToId("feed-content"),setTimeout(()=>o("textarea").trigger("focus").val(` || @${d.user.name} : ${d.content}`).trigger("input"),50)}))}}),e.source==="shy"&&(h.feedPage++,o("#feed-more").children("a").text("点击查看更多..."))):i()};let l=o(".feed-selector");o('<li class="feed-selector" id="exlg-benben-selector" data-mode="all-exlg" exlg="exlg"><a style="cursor: pointer">全网动态</a></li>').appendTo(l.parent()).on("click",s=>{let a=o(s.currentTarget);l.removeClass("am-active"),a.addClass("am-active"),e.source==="o2"&&o("#feed-more").hide(),h.feedPage=1,h.feedMode="all-exlg",o("li.am-comment").remove(),h.loadFeed()})});c.reg_board("search-user","查找用户名",null,({$board:e})=>{e.html(`
        <h3>查找用户</h3>
...
`);c.reg("keyboard-and-cli","键盘操作与命令行","@/.*",{lang:{ty:"enum",dft:"en",vals:["en","zh"]}},({msto:e})=>{let t=o('<div id="exlg-cli" exlg="exlg"></div>').appendTo(o("body")),r=o('<input id="exlg-cli-input" />').appendTo(t),n=!1,i=(p,...f)=>{n=!0;let m=p.map((w,x)=>w.split(/\b/).map(v=>u[v]?.[_-1]??v).join("")+(f[x]||"")).join("");return r.val(m)},l=(p,...f)=>I(i(p,...f).addClass("error").val()),s=()=>(n=!1,r.val("").removeClass("error")),a=[],d=0,g=["en","zh"],u={".":["。"],",":["，"],"!":["！"],"?":["？"],cli:["命令行"],current:["当前"],language:["语言"],available:["可用"],command:["命令"],commands:["命令"],unknown:["未知"],forum:["板块"],target:["目标"],mod:["模块"],action:["操作"],illegal:["错误"],param:["参数"],expected:["期望"],type:["类型"],lost:["缺失"],essential:["必要"],user:["用户"]},_=g.indexOf(e.lang)||0,y=(p,f,m,w)=>(f=f.replace(/ /g,"").split(",").map(x=>{let v={};return x[0]==="["?(v.essential=!1,x=x.slice(1,-1)):v.essential=!0,[v.name,v.type]=x.split(":"),v}),{name:p,arg:f,help:m,fn:w}),C=[y("help","[cmd: string]",["get the help of <cmd>. or list all cmds.","获取 <cmd> 的帮助。空则列出所有"],p=>{if(!p)i`exlg cli. current language: ${_}, available commands: ${Object.keys(C).join(", ")}`;else{let f=C[p];if(!f)return l`help: unknown command "${p}"`;let m=f.arg.map(w=>{let x=w.name+": "+w.type;return w.essential?`<${x}>`:`[${x}]`}).join(" ");i`${p} ${m} ${f.help[_]}`}}),y("cd","path: string",["jump to <path>, relative path is OK.","跳转至 <path>，支持相对路径。"],p=>{let f;if(p[0]==="/")f=p;else{let m=location.pathname.replace(/^\/+/,"").split("/");p.split("/").forEach(x=>{x!=="."&&(x===".."?m.pop():m.push(x))}),f=m.join("/")}location.href=location.origin+"/"+f.replace(/^\/+/,"")}),y("cdd","forum: string",["jump to the forum named <forum> of discussion. use all the names you can think of.","跳转至名为 <forum> 的讨论板块，你能想到的名字基本都有用。"],p=>{let f=[["relevantaffairs","gs","gsq","灌水","灌水区","r","ra"],["academics","xs","xsb","学术","学术版","a","ac"],["siteaffairs","zw","zwb","站务","站务版","s","sa"],["problem","tm","tmzb","题目","题目总版","p"],["service","fk","fksqgd","反馈","反馈、申请、工单专版","se"]];if(p=f.find(m=>m.includes(p))?.[0],!f)return l`cdd: unknown forum "${p}"`;C.cd.fn(`/discuss/lists?forumname=${p}`)}),y("cc","[name: char]",['jump to <name>, "h|p|c|r|d|i|m|n" stands for home|problem|contest|record|discuss|I myself|message|notification. or jump home.','跳转至 [name]，"h|p|c|r|d|i|m|n" 代表：主页|题目|比赛|评测记录|讨论|个人中心|私信|通知。空则跳转主页。'],p=>{p=p||"h";let f={h:"/",p:"/problem/list",c:"/contest/list",r:"/record/list",d:"/discuss/lists",i:"/user/"+N.uid,m:"/chat",n:"/user/notification"}[p];f?C.cd.fn(f):l`cc: unknown target "${p}"`}),y("mod","action: string, [name: string]",['for <action> "enable|disable|toggle", opearte the mod named <name>.','当 <action> 为 "enable|disable|toggle"，对名为 <name> 的模块执行对应操作：启用|禁用|切换。'],(p,f)=>{switch(p){case"enable":case"disable":case"toggle":if(!c.has(f))return l`mod: unknown mod "${f}"`;L[f].on={enable:()=>!0,disable:()=>!1,toggle:m=>!m}[p](L[f].on);break;default:return l`mod: unknown action "${p}"`}}),y("dash","action: string",['for <action> "show|hide|toggle", opearte the exlg dashboard.','当 <action> 为 "show|hide|toggle", 显示|隐藏|切换 exlg 管理面板。'],p=>{if(!["show","hide","toggle"].includes(p))return l`dash: unknown action "${p}"`;o("#exlg-dash-window")[p]()}),y("lang","lang: string",['for <lang> "en|zh" switch current cli language.','当 <lang> 为 "en|zh"，切换当前语言。'],p=>{try{e.lang=p,_=g.indexOf(p)}catch{return l`lang: unknown language ${p}`}}),y("uid","uid: integer",["jumps to homepage of user whose uid is <uid>.","跳转至 uid 为 <uid> 的用户主页。"],p=>location.href=`/user/${p}`),y("un","name: string",["jumps to homepage of user whose username is like <name>.","跳转至用户名与 <name> 类似的用户主页。"],p=>{o.get("/api/user/search?keyword="+p,f=>{f.users[0]?location.href="/user/"+f.users[0].uid:l`un: unknown user "${p}".`})})].reduce((p,f)=>(p[f.name]=f,p),{}),j=p=>{$(`Parsing command: "${p}"`);let f=p.trim().replace(/^\//,"").split(" "),m=f.shift();if(!m)return;let w=C[m];if(!w)return l`exlg: unknown command "${m}"`;let x=-1,v;for([x,v]of f.entries()){let k=w.arg[x].type;if((k==="number"||k==="integer")&&(f[x]=+v),!(k==="char"&&v.length===1||k==="number"&&!isNaN(f[x])||k==="integer"&&!isNaN(f[x])&&!(f[x]%1)||k==="string"))return l`${m}: illegal param "${v}", expected type ${k}.`}if(w.arg[x+1]?.essential)return l`${m}: lost essential param "${w.arg[x+1].name}"`;w.fn(...f)};r.on("keydown",p=>{switch(p.key){case"Enter":if(n)return s();let f=r.val();if(a.push(f),d=a.length,j(f),!n)return s();break;case"/":n&&s();break;case"Escape":t.hide();break;case"ArrowUp":case"ArrowDown":let m=d+{ArrowUp:-1,ArrowDown:1}[p.key];if(m<0||m>=a.length)return;d=m,r.val(a[m]);break}}),o(h).on("keydown",p=>{let f=o(document.activeElement);if(f.is("body")){let m={ArrowLeft:"prev",ArrowRight:"next"}[p.key];if(m)return o(`a[rel=${m}]`)[0].click();if(p.shiftKey){let w={ArrowUp:0,ArrowDown:1e6}[p.key];w!==void 0&&h.scrollTo(0,w)}p.key==="/"&&(t.show(),s().trigger("focus"))}else f.is("[name=captcha]")&&p.key==="Enter"&&o("#submitpost, #submit-reply")[0].click()})},`
    #exlg-cli {
        position: fixed;
        top: 0;
        z-index: 65536;
...
`);c.reg_hook_new("rand-training-problem","题单内随机跳题","@/training/[0-9]+(#.*)?",{mode:{ty:"enum",vals:["unac only","unac and new","new only"],dft:"unac and new",info:["Preferences about problem choosing","随机跳题的题目种类"]}},({msto:e,args:t})=>{let r=e.mode.startsWith("unac")+e.mode.endsWith("only")*-1+2;!t.length||o(t[0].firstChild).clone(!0).appendTo(t).text("随机跳题").addClass("exlg-rand-training-problem-btn").on("click",()=>{let n=b.training,i=[];if(n.problems.some(s=>{n.userScore.score[s.problem.pid]===null?r&1&&i.push(s.problem.pid):n.userScore.score[s.problem.pid]<s.problem.fullScore&&r&2&&i.push(s.problem.pid)}),n.problemCount){if(!i.length)return r===1?T("您已经做完所有新题啦！"):r===2?T("您已经订完所有错题啦！"):T("您已经切完所有题啦！")}else return T("题单不能为空");let l=~~(Math.random()*1e6)%i.length;location.href="https://www.luogu.com.cn/problem/"+i[l]})},e=>{let t=o(e.target).find("div.operation");return{result:t.length>0,args:t}},()=>o("div.operation"),`
.exlg-rand-training-problem-btn {
    border-color: rgb(52, 52, 52);
    background-color: rgb(52, 52, 52);
}
`);c.reg_chore("sponsor-list","获取标签列表","1D","@/.*",{tag_list:{ty:"string",priv:!0}},({msto:e})=>{M({url:"https://service-cmrlfv7t-1305163805.sh.apigw.tencentcs.com/release/get/0/0/",onload:t=>{e.tag_list=decodeURIComponent(t.responseText)}})});c.reg_hook_new("sponsor-tag","标签显示",["@/","@/paste","@/discuss/.*","@/problem/.*","@/ranking.*"],{tag_list:{ty:"string",priv:!0}},({args:e})=>{let t=JSON.parse(L["^sponsor-list"].tag_list),r=n=>{if(!n||n.hasClass("exlg-badge-username")||!/\/user\/[1-9][0-9]{0,}/.test(n.attr("href")))return;n.addClass("exlg-badge-username");let i=n.attr("href").slice(6),l=t[i];if(!l)return;let s=o(i==="100250"?`<span class="am-badge am-radius lg-bg-red" style="margin-left: 4px;">${l}</span>`:`<span class="exlg-badge">${l}</span>`).off("contextmenu").on("contextmenu",()=>!1).on("mousedown",d=>{d.button===2?location.href="https://www.luogu.com.cn/paste/asz40850":d.button===0&&Z()}),a=n;a.next().length&&a.next().hasClass("sb_amazeui")&&(a=a.next()),a.next().length&&a.next().hasClass("am-badge")&&(a=a.next()),a.after(s)};e.each((n,i)=>r(o(i)))},e=>{let t=o(e.target).find("a[target='_blank'][href]");return{result:t.length,args:t}},()=>o("a[target='_blank'][href]"),`
.exlg-badge {
    border-radius: 50px;
    padding-left: 10px;
    padding-right: 10px;
...
`);c.reg("virtual-participation","创建重现赛","@/contest/[0-9]*(#.*)?",{},()=>{if(b.contest.name.match("Virtual Participation")){debugger;let e=o(".items"),t=()=>{(location.hash||"#main")==="#problems"&&o(".pid").length!==0&&b.contest.startTime>K(1e3)&&(o("a.title.color-default").on("click",()=>{T("比赛尚未开始, 请开始后再查看题目")}),o("a.title.color-default").removeAttr("href"))};e.on("click",t),t();return}if(b.contest.endTime>K(1e3)){I("Contest has not started or ended.");return}debugger;o("<button id='exlg-vp' class='lfe-form-sz-middle'>重现比赛</button>").appendTo(o("div.operation")).click(async()=>{T(`<div>
                ...
                <p>开始时间：<input type="date" id="vpTmDt"/> <input type="time" id="vpTmClk"/></p>
            </div><br>`,"创建重现赛",async()=>{let e=o("#vpTmDt")[0].value.split("-"),t=o("#vpTmClk")[0].value.split(":"),r=new Date(e[0],e[1]-1,e[2],t[0],t[1],0,0);r=r.getTime()/1e3;let n="",i="";o.each(b.contestProblems,(g,u)=>{g&&(n+=",",i+=","),n+='"'+u.problem.pid+'"',i+=`"${u.problem.pid}": ${u.problem.fullScore}`});let l=null,s=`{
                    "settings":{
                    ...
                    "hostID":${N.uid}
                }`;l=await U("/fe/api/contest/new",s);let a=l.id.toString();switch(l.status??200){case 200:a=l.id.toString();break;default:E(`Failed to modify contest ${a} with status code ${l.status}.`)}l=null;try{l=await U(`/fe/api/contest/editProblem/${a}`,`{
                            "pids":[${n}],
                            "scores":{${i}}
                        }`)}catch{T("<p>本场比赛的题目不公开</p>","重现赛创建失败");return}let d=await P(`/contest/edit/${a}`);await U(`/fe/api/contest/join/${a}`,`{"code": "${d.currentData.contest.joinCode}"}`),T(`<p>邀请码: ${d.currentData.contest.joinCode}</p>
