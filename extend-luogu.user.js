...
// ==/UserScript==
;
(()=>{var h=unsafeWindow,$=(e,...t)=>h.console.log(`%c[exlg] ${e}`,"color: #0e90d2;",...t),D=(e,...t)=>h.console.warn(`%c[exlg] ${e}`,"color: #0e90d2;",...t),M=(e,...t)=>{throw h.console.error(`%c[exlg] ${e}`,"color: #0e90d2;",...t),Error(t.join(" "))},b=null,S=null;location.host==="www.luogu.com.cn"&&!/blog/g.test(location.href)&&(/(\?|&)_contentOnly($|=)/g.test(location.search)&&M("Content-Only pages."),h._feInjection.code!==200&&M("Luogu failed to load. Exlg stops loading."),b=h._feInjection.currentData,S=h._feInjection.currentUser);var o=jQuery,q=new filterXSS.FilterXSS({onTagAttr:(e,t,n)=>{if(t==="style")return`${t}="${n}"`}});o.double=(e,t,n)=>[e(t),e(n)];Date.prototype.format=function(e,t){t=t?"UTC":"";let n={"y+":this[`get${t}FullYear`](),"m+":this[`get${t}Month`]()+1,"d+":this[`get${t}Date`](),"H+":this[`get${t}Hours`](),"M+":this[`get${t}Minutes`](),"S+":this[`get${t}Seconds`](),"s+":this[`get${t}Milliseconds`]()};for(let r in n)RegExp(`(${r})`).test(e)&&(e=e.replace(RegExp.$1,("000"+n[r]).substr(n[r].toString().length+3-RegExp.$1.length)));return e};String.prototype.toInitialCase=function(){return this[0].toUpperCase()+this.slice(1)};Array.prototype.lastElem=function(){return this[this.length-1]};var R=(e,t)=>{if(!e)return"<<";let n=(u,w)=>u===w?"==":u<w?"<<":">>",r=["pre","alpha","beta"],[[i,l],[a,s]]=[e,t].map(u=>u.split(" "));if(i===a)return n(...[l,s].map(u=>u?r.findIndex(w=>w===u):1/0));let[d,p]=[i,a].map(u=>u.split("."));for(let[u,w]of d.entries())if(w!==p[u])return n(+w||0,+p[u]||0)},K=(e=1)=>~~(new Date().getTime()/e),P=e=>new Promise((t,n)=>o.get(e+(e.includes("?")?"&":"?")+"_contentOnly=1",r=>{r.code!==200&&n(`Requesting failure code: ${t.code}.`),t(r)})),B=h.show_alert?(e,t="exlg 提醒您")=>h.show_alert(t,e):(e,t="exlg 提醒您")=>h.alert(t+`
`+e),X=null,H=(e,t)=>o.ajax({url:e,data:t,headers:{"x-csrf-token":X===null?X=o("meta[name=csrf-token]").attr("content"):X,"content-type":"application/json"},method:"post"}),T=({url:e,onload:t,onerror:n=r=>M(r)})=>GM_xmlhttpRequest({url:e,method:"GET",onload:t,onerror:n}),I=(e,t)=>{let n=new URLSearchParams;for(let i in e)n.set(i,e[i]);let r=o(`
        <iframe id="exlg-${e.type}" src=" https://www.bilibili.com/robots.txt?${n}" style="${t}" exlg="exlg"></iframe>
    `);return $("Building springboard: %o",r[0]),r},J=e=>[/^AT[1-9][0-9]{0,}$/i,/^CF[1-9][0-9]{0,}[A-Z][0-9]?$/i,/^SP[1-9][0-9]{0,}$/i,/^P[1-9][0-9]{3,}$/i,/^UVA[1-9][0-9]{2,}$/i,/^U[1-9][0-9]{0,}$/i,/^T[[1-9][0-9]{0,}$/i].some(t=>t.test(e)),G={_ac_func:null,wrapper:null,container:null,wait_time:null,header:null,content:null,autoquit:!0,show_dialog(){this.wrapper.css("display","flex"),setTimeout(()=>{this.container.removeClass("container-hide"),this.container.addClass("container-show")},50)},hide_dialog(){this.container.addClass("container-hide"),this.container.removeClass("container-show"),setTimeout(()=>this.wrapper.hide(),this.wait_time)},accept_dialog(){this._ac_func(this.hide_dialog),this.autoquit&&this.hide_dialog()},show_exlg_alert(e="",t="exlg 提醒您",n=()=>{},r=!0){this.autoquit=r,this._ac_func=n,this.header.text(t),this.content.html(e),this.show_dialog()}},A=(...e)=>G.show_exlg_alert(...e),Z=async()=>{let e=(n,r)=>T({url:n,onload:i=>{let l=-1,a=i.response;[{errorcode:0,message:'"Succeed in creating a new badge!"',ontitle:"[exlg] 成功创建 badge",onlog:"Successfully created a badge!"},{errorcode:1,message:'"Wrong active code!"',ontitle:"无效激活码",onlog:"Illegal Active Code"},{errorcode:2,message:'"Sorry, but the active code has been used!"',ontitle:"激活码已被使用",onlog:"Expired Active Code"},{errorcode:3,message:'"Something went wrong!"',ontitle:"非法的 badge 内容",onlog:"Illegal Badge Text"},{errorcode:-1,message:"Fuck CCF up",ontitle:"未知错误",onlog:"注册 exlg-badge 时出现未知错误, 请联系开发人员"}].forEach(d=>{if(l===-1&&(console.log(a===d.message,d.errorcode===-1,d),(a===d.message||d.errorcode===-1)&&(l=d.errorcode,o("#exlg-dialog-title").html(d.errorcode?`[Error] ${d.ontitle}`:d.ontitle),$(d.errorcode?`Illegal Operation in registering badge: ${d.onlog}(#${d.errorcode})`:d.onlog),l===-1||!l))){r(),setTimeout(()=>A("badge 注册成功!","exlg 提醒您"),400);return}})}}),t="exlg badge 注册器 ver.5.0";A(`<div class="exlg-update-log-text exlg-unselectable exlg-badge-page" style="font-family: Consolas;">
    ...
    `,t,n=>{let r=o("#exlg-container"),i=r.find("input"),l=r.find("#exlg-dialog-title");if(h._feInjection&&S&&S.uid&&!i[0].value&&(i[0].value=S.uid),!(i[0].value&&i[1].value&&i[2].value)){l.html("[Err] 请检查信息是否填写完整"),setTimeout(()=>l.html(t),1500);return}o.get("/api/user/search?keyword="+i[0].value,a=>{if(!a.users[0])l.html("[Err] 无法找到指定用户"),setTimeout(()=>l.html(t),1500);else{i[0].value=a.users[0].uid;let s=`https://service-cmrlfv7t-1305163805.sh.apigw.tencentcs.com/release/${i[1].value}/${i[0].value}/${i[2].value}/`;e(s,n)}})},!1)};var L=null,c={_:new Map,fake_sto:L,data:{},path_alias:[["",".*\\.luogu\\.(com\\.cn|org)"],["dash","dash.exlg.cc"],["cdn","cdn.luogu.com.cn"],["bili","www.bilibili.com"],["tcs1","service-ig5px5gh-1305163805.sh.apigw.tencentcs.com"],["tcs2","service-nd5kxeo3-1305163805.sh.apigw.tencentcs.com"],["tcs3","service-otgstbe5-1305163805.sh.apigw.tencentcs.com"],["debug","localhost:1634"],["ghpage","extend-luogu.github.io"]].map(([e,t])=>[new RegExp(`^@${e}/`),t]),path_dash_board:["@dash/((index|bundle)(.html)?)?","@ghpage/exlg-setting-new/((index|bundle)(.html)?)?","@debug/exlg-setting/((index|bundle).html)?"],reg:(e,t,n,r,i,l)=>{Array.isArray(n)||(n=[n]),n.forEach((a,s)=>{c.path_alias.some(([d,p])=>{if(a.match(d))return n[s]=a.replace(d,p+"/"),!0}),a.endsWith("$")||(n[s]+="$")}),c.data[e]={ty:"object",lvs:{...r,on:{ty:"boolean",dft:!0}}},c._.set(e,{info:t,path:n,func:i,styl:l})},reg_pre:(e,t,n,r,i,l,a)=>{c.reg(e,t,n,r,l,a),c._.set(e,{pre:i,...c._.get(e)})},reg_main:(e,t,n,r,i,l)=>c.reg("@"+e,t,n,r,a=>(i(a),!1),l),reg_user_tab:(e,t,n,r,i,l,a)=>c.reg(e,t,"@/user/.*",i,s=>{let d=o(".items"),p=()=>{(location.hash||"#main")==="#"+n&&($(`Working user tab#${n} mod: "${e}"`),l({...s,vars:r}))};d.on("click",p),p()},a),reg_chore:(e,t,n,r,i,l,a)=>{if(typeof n=="string"){let s=+n.slice(0,-1),d={s:1e3,m:1e3*60,h:1e3*60*60,D:1e3*60*60*24}[n.slice(-1)];!isNaN(s)&&d?n=s*d:M(`Parsing period failed: "${n}"`)}e="^"+e,i={...i,last_chore:{ty:"number",dft:-1,priv:!0}},c.reg(e,t,r,i,s=>{let d=L[e].last_chore,p=Date.now(),u=!0;s.named||!d||p-d>n?(u&&(GM_addStyle(a),u=!1),l(s),L[e].last_chore=Date.now()):$(`Pending chore: "${e}"`)})},reg_board:(e,t,n,r,i)=>c.reg(e,t,"@/",n,l=>{let a=`
            ...
            `,s=o("#exlg-board");s.length||(s=o(`
                ...
                `).prependTo(".lg-right.am-u-md-4"),s[0].firstChild.style["font-size"]="1em"),r({...l,$board:o("<div></div>").appendTo(s)})},i),reg_hook:(e,t,n,r,i,l,a)=>c.reg(e,t,n,r,s=>{i(s),o("body").bind("DOMNodeInserted",d=>l(d)&&i(s))},a),reg_hook_new:(e,t,n,r,i,l,a,s)=>c.reg(e,t,n,r,d=>{i({...d,result:!1,args:a()}),o("body").bind("DOMNodeInserted",p=>{let u=l(p);return u.result&&i({...d,...u})})},s),find:e=>c._.get(e),has:e=>c._.has(e),disable:e=>{let t=c.find(e);t.on=!1,c._.set(e,t)},enable:e=>{let t=c.find(e);t.on=!0,c._.set(e,t)},preload:e=>{L===null&&(L=c.fake_sto);let t=(r,i)=>{r||M(`Preloading named mod but not found: "${e}"`),$(`Preloading ${i?"named ":""}mod: "${r.name}"`);try{return{pred:r.pre({msto:L[r.name],named:i}),...r}}catch(l){return D(l),r}},n=location.href;for(let[r,i]of c._.entries())L[r].on&&i.path.some(l=>new RegExp(l,"g").test(n))&&(i.willrun=!0,"pre"in i&&c._.set(r,t({name:r,...i})))},execute:e=>{let t=(n,r)=>{n||M(`Executing named mod but not found: "${e}"`),n.styl&&GM_addStyle(n.styl),$(`Executing ${r?"named ":""}mod: "${n.name}"`);try{return"pred"in n?n.func({msto:L[n.name],named:r,pred:n.pred}):n.func({msto:L[n.name],named:r})}catch(i){D(i)}};if(e){let n=c.find(e);return t(n,!0)}for(let[n,r]of c._.entries())if(r.on=L[n].on,r.willrun&&t({name:n,...r})===!1)break}};c.reg("benben","全网犇犇","@/",{source:{ty:"enum",dft:"o2",vals:["o2","shy"],info:["Switch the way of fetching benben","切换全网犇犇获取方式"]}},({msto:e})=>{let t={Gray:"gray",Blue:"bluelight",Green:"green",Orange:"orange lg-bold",Red:"red lg-bold",Purple:"purple lg-bold",Brown:"brown lg-bold"},n=`
    ...
    `,r=a=>a<=3?"":n.replace("%",a<=5?"#5eb95e":a<=7?"#3498db":"#f1c40f"),i=h.loadFeed;h.loadFeed=function(){h.feedMode==="all-exlg"?(T({url:e.source==="o2"?"https://service-ig5px5gh-1305163805.sh.apigw.tencentcs.com/release/APIGWHtmlDemo-1615602121":`https://bens.rotriw.com/api/list/proxy?page=${h.feedPage}`,onload:a=>{JSON.parse(a.response).forEach(d=>o(`
...
`);c.reg("keyboard-and-cli","键盘操作与命令行","@/.*",{lang:{ty:"enum",dft:"en",vals:["en","zh"]}},({msto:e})=>{let t=o('<div id="exlg-cli" exlg="exlg"></div>').appendTo(o("body")),n=o('<input id="exlg-cli-input" />').appendTo(t),r=!1,i=(g,...f)=>{r=!0;let m=g.map((_,x)=>_.split(/\b/).map(v=>u[v]?.[w-1]??v).join("")+(f[x]||"")).join("");return n.val(m)},l=(g,...f)=>D(i(g,...f).addClass("error").val()),a=()=>(r=!1,n.val("").removeClass("error")),s=[],d=0,p=["en","zh"],u={".":["。"],",":["，"],"!":["！"],"?":["？"],cli:["命令行"],current:["当前"],language:["语言"],available:["可用"],command:["命令"],commands:["命令"],unknown:["未知"],forum:["板块"],target:["目标"],mod:["模块"],action:["操作"],illegal:["错误"],param:["参数"],expected:["期望"],type:["类型"],lost:["缺失"],essential:["必要"],user:["用户"]},w=p.indexOf(e.lang)||0,y=(g,f,m,_)=>(f=f.replace(/ /g,"").split(",").map(x=>{let v={};return x[0]==="["?(v.essential=!1,x=x.slice(1,-1)):v.essential=!0,[v.name,v.type]=x.split(":"),v}),{name:g,arg:f,help:m,fn:_}),C=[y("help","[cmd: string]",["get the help of <cmd>. or list all cmds.","获取 <cmd> 的帮助。空则列出所有"],g=>{if(!g)i`exlg cli. current language: ${w}, available commands: ${Object.keys(C).join(", ")}`;else{let f=C[g];if(!f)return l`help: unknown command "${g}"`;let m=f.arg.map(_=>{let x=_.name+": "+_.type;return _.essential?`<${x}>`:`[${x}]`}).join(" ");i`${g} ${m} ${f.help[w]}`}}),y("cd","path: string",["jump to <path>, relative path is OK.","跳转至 <path>，支持相对路径。"],g=>{let f;if(g[0]==="/")f=g;else{let m=location.pathname.replace(/^\/+/,"").split("/");g.split("/").forEach(x=>{x!=="."&&(x===".."?m.pop():m.push(x))}),f=m.join("/")}location.href=location.origin+"/"+f.replace(/^\/+/,"")}),y("cdd","forum: string",["jump to the forum named <forum> of discussion. use all the names you can think of.","跳转至名为 <forum> 的讨论板块，你能想到的名字基本都有用。"],g=>{let f=[["relevantaffairs","gs","gsq","灌水","灌水区","r","ra"],["academics","xs","xsb","学术","学术版","a","ac"],["siteaffairs","zw","zwb","站务","站务版","s","sa"],["problem","tm","tmzb","题目","题目总版","p"],["service","fk","fksqgd","反馈","反馈、申请、工单专版","se"]];if(g=f.find(m=>m.includes(g))?.[0],!f)return l`cdd: unknown forum "${g}"`;C.cd.fn(`/discuss/lists?forumname=${g}`)}),y("cc","[name: char]",['jump to <name>, "h|p|c|r|d|i|m|n" stands for home|problem|contest|record|discuss|I myself|message|notification. or jump home.','跳转至 [name]，"h|p|c|r|d|i|m|n" 代表：主页|题目|比赛|评测记录|讨论|个人中心|私信|通知。空则跳转主页。'],g=>{g=g||"h";let f={h:"/",p:"/problem/list",c:"/contest/list",r:"/record/list",d:"/discuss/lists",i:"/user/"+S.uid,m:"/chat",n:"/user/notification"}[g];f?C.cd.fn(f):l`cc: unknown target "${g}"`}),y("mod","action: string, [name: string]",['for <action> "enable|disable|toggle", opearte the mod named <name>.','当 <action> 为 "enable|disable|toggle"，对名为 <name> 的模块执行对应操作：启用|禁用|切换。'],(g,f)=>{switch(g){case"enable":case"disable":case"toggle":if(!c.has(f))return l`mod: unknown mod "${f}"`;L[f].on={enable:()=>!0,disable:()=>!1,toggle:m=>!m}[g](L[f].on);break;default:return l`mod: unknown action "${g}"`}}),y("dash","action: string",['for <action> "show|hide|toggle", opearte the exlg dashboard.','当 <action> 为 "show|hide|toggle", 显示|隐藏|切换 exlg 管理面板。'],g=>{if(!["show","hide","toggle"].includes(g))return l`dash: unknown action "${g}"`;o("#exlg-dash-window")[g]()}),y("lang","lang: string",['for <lang> "en|zh" switch current cli language.','当 <lang> 为 "en|zh"，切换当前语言。'],g=>{try{e.lang=g,w=p.indexOf(g)}catch{return l`lang: unknown language ${g}`}}),y("uid","uid: integer",["jumps to homepage of user whose uid is <uid>.","跳转至 uid 为 <uid> 的用户主页。"],g=>location.href=`/user/${g}`),y("un","name: string",["jumps to homepage of user whose username is like <name>.","跳转至用户名与 <name> 类似的用户主页。"],g=>{o.get("/api/user/search?keyword="+g,f=>{f.users[0]?location.href="/user/"+f.users[0].uid:l`un: unknown user "${g}".`})})].reduce((g,f)=>(g[f.name]=f,g),{}),j=g=>{$(`Parsing command: "${g}"`);let f=g.trim().replace(/^\//,"").split(" "),m=f.shift();if(!m)return;let _=C[m];if(!_)return l`exlg: unknown command "${m}"`;let x=-1,v;for([x,v]of f.entries()){let k=_.arg[x].type;if((k==="number"||k==="integer")&&(f[x]=+v),!(k==="char"&&v.length===1||k==="number"&&!isNaN(f[x])||k==="integer"&&!isNaN(f[x])&&!(f[x]%1)||k==="string"))return l`${m}: illegal param "${v}", expected type ${k}.`}if(_.arg[x+1]?.essential)return l`${m}: lost essential param "${_.arg[x+1].name}"`;_.fn(...f)};n.on("keydown",g=>{switch(g.key){case"Enter":if(r)return a();let f=n.val();if(s.push(f),d=s.length,j(f),!r)return a();break;case"/":r&&a();break;case"Escape":t.hide();break;case"ArrowUp":case"ArrowDown":let m=d+{ArrowUp:-1,ArrowDown:1}[g.key];if(m<0||m>=s.length)return;d=m,n.val(s[m]);break}}),o(h).on("keydown",g=>{let f=o(document.activeElement);if(f.is("body")){let m={ArrowLeft:"prev",ArrowRight:"next"}[g.key];if(m)return o(`a[rel=${m}]`)[0].click();if(g.shiftKey){let _={ArrowUp:0,ArrowDown:1e6}[g.key];_!==void 0&&h.scrollTo(0,_)}g.key==="/"&&(t.show(),a().trigger("focus"))}else f.is("[name=captcha]")&&g.key==="Enter"&&o("#submitpost, #submit-reply")[0].click()})},`
    #exlg-cli {
        position: fixed;
        top: 0;
        z-index: 65536;
...
`);c.reg_hook_new("submission-color","记录难度可视化","@/record/list.*",null,async({args:e})=>{if(e&&e.type==="show"){if(o("div.problem > div > a > span.pid").length&&!o(".exlg-difficulty-color").length){let i=(await P(location.href)).currentData.records.result.map(l=>l.problem.difficulty);o("div.problem > div > a > span.pid").each((l,a,s=o(a))=>{s.addClass("exlg-difficulty-color").addClass(`color-${i[l]}`)})}return}if(o(".exlg-difficulty-color").length)return;let n=(await P(location.href)).currentData.records.result.map(r=>r.problem.difficulty);o(e.target).find("div.problem > div > a > span.pid").each((r,i,l=o(i))=>{l.addClass("exlg-difficulty-color").addClass(`color-${n[r]}`)})},e=>{let t=e.target;return!t||!t.tagName?{args:z.COMMENT_TAG,result:!1}:t.tagName.toLowerCase()==="a"&&(t.href||"").includes("/problem/")&&` ${t.parentNode.parentNode.className} `.includes(" problem ")?t.parentNode.parentNode.parentNode.nextSibling?{args:{type:"modified - not the last one.",target:null},result:!1}:{args:{type:"modified - update",target:t.parentNode.parentNode.parentNode.parentNode},result:!0}:{args:{type:"modified - not that one.",target:null},result:!1}},()=>({type:"show"}),"");c.reg("mainpage-discuss-limit","主页讨论个数限制",["@/"],{max_discuss:{ty:"number",dft:12,min:4,max:16,step:1,info:["Max Discussions On Show","主页讨论显示上限"],strict:!0}},({msto:e})=>{let t;location.href.includes("blog")||(o(".lg-article").each((n,r,i=o(r))=>{let l=r.childNodes[1];l&&l.tagName.toLowerCase()==="h2"&&l.innerText.includes("讨论")&&(t=i.children(".am-panel"))}),t.each((n,r,i=o(r))=>{n>=e.max_discuss&&i.hide()}))});c.reg("user-css","自定义样式表",".*",{css:{ty:"string"}},({msto:e})=>GM_addStyle(e.css));c.reg_chore("atdiff-fetch","获取_AtCoder_难度","10D","@/problem/AT.*",{atdiff:{ty:"string",priv:!0}},({msto:e})=>{let t={};T({url:"https://kenkoooo.com/atcoder/resources/problem-models.json",onload:n=>{let r=JSON.parse(n.responseText);for(let i in r)t[i]=r[i].difficulty;e.atdiff=JSON.stringify(t)}})});c.reg_pre("original-difficulty","显示原始难度",["@/problem/CF.*","@/problem/AT.*"],{cf_src:{ty:"enum",dft:"codeforces.com",vals:["codeforces.com","codeforces.ml"],info:["Codeforces problem source","CF 题目源"]}},async({msto:e})=>new Promise((t,n)=>{let r=location.pathname.match(/(CF|AT)([0-9]|[A-Z])*$/g)[0].substring(2);if(location.pathname.includes("CF")){let i=r.match(/^[0-9]*/g)[0],l=r.substring(i.length);T({url:`https://${e.cf_src}/problemset/problem/${i}/${l}`,onload:a=>{let s=o(a.responseText).find("span[title=Difficulty]").text().trim();t(s?s.substring(1):void 0)},onerror:a=>{M(a),n(a)}})}else{let i=JSON.parse(L["^atdiff-fetch"].atdiff),l=b.problem.description.match(RegExp("^.{22}[-./A-Za-z0-9_]*"))[0].match(RegExp("[^/]*$"));l in i?t(Math.round(i[l]>=400?i[l]:400/Math.exp(1-i[l]/400))):t(void 0)}}),({pred:e})=>{let t=document.querySelectorAll("div.field"),n=t[3].cloneNode(!0);t[3].after(n);let r=n.querySelectorAll("span");r[0].innerText="原始难度",r[1].innerText="获取中",e.then(i=>{i===void 0&&(i="不可用"),r[1].innerText=i})});c.reg("rand-problem-ex","随机跳题_ex","@/",{exrand_difficulty:{ty:"tuple",lvs:[{ty:"boolean",dft:!1,strict:!0,repeat:8}],priv:!0},exrand_source:{ty:"tuple",lvs:[{ty:"boolean",dft:!1,strict:!0,repeat:5}],priv:!0}},({msto:e})=>{let t=[["入门","red"],["普及-","orange"],["普及/提高-","yellow"],["普及+/提高","green"],["提高+/省选-","blue"],["省选/NOI-","purple"],["NOI/NOI+/CTSC","black"],["暂无评定","gray"]].map((m,_,x)=>({text:m[0],color:m[1],id:(_+1)%x.length})),n=[{text:"洛谷题库",color:"red",id:"P"},{text:"Codeforces",color:"orange",id:"CF"},{text:"SPOJ",color:"yellow",id:"SP"},{text:"AtCoder",color:"green",id:"AT"},{text:"UVA",color:"blue",id:"UVA"}],r=m=>{J(m)&&(m=m.toUpperCase()),m===""||typeof m>"u"?h.show_alert("提示","请输入题号"):location.href="https://www.luogu.com.cn/problemnew/show/"+m},i=!1,l=!1,a=o("input[name='toproblem']");a.after(a.clone()).remove(),a=o("input[name='toproblem']");let s=o(".am-btn[name='goto']");s.after(s.clone()).remove(),s=o(".am-btn[name='goto']");let d=s.parent();o(".am-btn[name='gotorandom']").text("随机");let p=o('<button class="am-btn am-btn-success am-btn-sm" name="gotorandomex">随机ex</button>').appendTo(d);s.on("click",()=>{/^[0-9]+.?[0-9]*$/.test(a.val())&&a.val("P"+a.val()),r(a.val())}),a.on("keydown",m=>{m.keyCode===13&&s.click()});let u=o(`<span id="exlg-exrand-window" class="exlg-window" style="display: block;">
    <br>
    <ul></ul>
    </span>`).appendTo(d).hide().on("mouseenter",()=>{i=!0}).on("mouseleave",()=>{i=!1,l||u.hide()});o(".lg-index-stat>h2").text("问题跳转 ").append(o('<div id="exlg-dash-0" class="exlg-rand-settings">ex设置</div>'));let w=u.children("ul").css("list-style-type","none"),y=o('<div id="exlg-exrand-menu"></div>').appendTo(w);o("<br>").appendTo(w);let C=o('<div id="exlg-exrand-diff" class="smallbtn-list"></div>').appendTo(w),j=o('<div id="exlg-exrand-srce" class="smallbtn-list"></div>').appendTo(w).hide(),g=o.double(m=>o(`<div class="exlg-rand-settings exlg-unselectable exrand-entry">${m}</div>`).appendTo(y),"题目难度","题目来源");g[0].after(o('<span class="exlg-unselectable">&nbsp;&nbsp;</span>')),g[0].addClass("selected").css("margin-right","38px"),o.double(([m,_])=>{m.on("click",()=>{o(".exrand-entry").removeClass("selected"),m.addClass("selected"),o(".smallbtn-list").hide(),_.show()})},[g[0],C],[g[1],j]),o.double(([m,_,x])=>{let v=o.double(([k,E])=>o(`<span class="${k}">
        <span class="lg-small lg-inline-up exlg-unselectable">${E}</span>
        <br>
        </span>`).appendTo(m),["exrand-enabled","已选择"],["exrand-disabled","未选择"]);_.forEach((k,E)=>{let N=o.double(O=>o(`<div class="exlg-smallbtn exlg-unselectable">${k.text}</div>`).css("background-color",`var(--lg-${k.color}-problem)`).appendTo(O),v[0],v[1]);o.double(O=>{N[O].on("click",()=>{N[O].hide(),N[1-O].show(),x[E]=!!O}),x[E]===!!O&&N[O].hide()},0,1)})},[C,t,e.exrand_difficulty],[j,n,e.exrand_source]),o("#exlg-dash-0").on("mouseenter",()=>{l=!0,o.double(([m,_])=>{o.double(([x,v])=>{m.children(x).children(".exlg-smallbtn").each((k,E,N=o(E))=>_[k]===v?N.show():N.hide())},[".exrand-enabled",!0],[".exrand-disabled",!1])},[C,e.exrand_difficulty],[j,e.exrand_source]),u.show()}).on("mouseleave",()=>{l=!1,i||setTimeout(()=>{i||u.hide()},200)});let f=async()=>{let m=o.double(([le,ie,ae])=>{let U=[];return le.forEach((se,de)=>{ie[de]&&U.push(se.id)}),U.length||(U=ae),U[Math.floor(Math.random()*U.length)]},[t,e.exrand_difficulty,[0,1,2,3,4,5,6,7]],[n,e.exrand_source,["P"]]),_=await P(`/problem/list?difficulty=${m[0]}&type=${m[1]}&page=1`),x=_.currentData.problems.count,v=Math.ceil(x/50),k=Math.floor(Math.random()*v)+1;_=await P(`/problem/list?difficulty=${m[0]}&type=${m[1]}&page=${k}`);let E=_.currentData.problems.result,N=Math.floor(Math.random()*E.length),O=E[N].pid;location.href=`/problem/${O}`};p.on("click",f)},`
.exlg-rand-settings {
    position: relative;
    display: inline-block;
...
`);c.reg_hook_new("rand-training-problem","题单内随机跳题","@/training/[0-9]+(#.*)?",{mode:{ty:"enum",vals:["unac only","unac and new","new only"],dft:"unac and new",info:["Preferences about problem choosing","随机跳题的题目种类"]}},({msto:e,args:t})=>{let n=e.mode.startsWith("unac")+e.mode.endsWith("only")*-1+2;!t.length||o(t[0].firstChild).clone(!0).appendTo(t).text("随机跳题").addClass("exlg-rand-training-problem-btn").on("click",()=>{let r=b.training,i=[];if(r.problems.some(a=>{r.userScore.score[a.problem.pid]===null?n&1&&i.push(a.problem.pid):r.userScore.score[a.problem.pid]<a.problem.fullScore&&n&2&&i.push(a.problem.pid)}),r.problemCount){if(!i.length)return n===1?A("您已经做完所有新题啦！"):n===2?A("您已经订完所有错题啦！"):A("您已经切完所有题啦！")}else return A("题单不能为空");let l=~~(Math.random()*1e6)%i.length;location.href="https://www.luogu.com.cn/problem/"+i[l]})},e=>{let t=o(e.target).find("div.operation");return{result:t.length>0,args:t}},()=>o("div.operation"),`
...
`);c.reg_main("springboard","跨域跳板",["@bili/robots.txt?.*","@/robots.txt?.*"],null,()=>{let e=new URLSearchParams(location.search);switch(e.get("type")){case"update":T({url:"https://api.github.com/repos/extend-luogu/extend-luogu/tags?per_page=1",onload:n=>h.parent.postMessage(["update",JSON.parse(n.responseText)[0].name],"*")}),h.addEventListener("message",n=>{n.data.unshift("update"),h.parent.postMessage(n.data,"*")});break;case"page":let t=e.get("url");(!e.get("confirm")||confirm(`是否加载来自 ${t} 的页面？`))&&(document.body.innerHTML=`<iframe src="${t}" exlg="exlg"></iframe>`);break;case"dash":break}},`
    iframe {
        border: none;
        display: block;
        width: 100%;
...
`);c.reg("virtual-participation","创建重现赛","@/contest/[0-9]*(#.*)?",{vp_id:{ty:"string",dft:"0",priv:!0},orig_dat:{ty:"object",priv:!0,lvs:{pids:{ty:"string"},scrs:{ty:"string"}}}},({msto:e})=>{if(b.contest.id.toString()===e.vp_id){D("You cannot vp the virtual contest."),b.contestProblems.length===0&&setTimeout(()=>{H(`/fe/api/contest/editProblem/${e.vp_id}`,`{
                ...
                <p>开始时间：<input type="date" id="vpTmDt"/> <input type="time" id="vpTmClk"/></p>
            </div><br>`,"创建重现赛",async()=>{let t=o("#vpTmDt")[0].value.split("-"),n=o("#vpTmClk")[0].value.split(":"),r=new Date(t[0],t[1]-1,t[2],n[0],n[1],0,0);r=r.getTime()/1e3,e.orig_dat.pids=e.orig_dat.scrs="",o.each(b.contestProblems,(s,d)=>{s&&(e.orig_dat.pids+=",",e.orig_dat.scrs+=","),e.orig_dat.pids+='"'+d.problem.pid+'"',e.orig_dat.scrs+=`"${d.problem.pid}": ${d.problem.fullScore}`});let i=e.vp_id==="0",l=`{
                    "settings":{
                    ...
                    "hostID":${S.uid}
                }`,a=await H(`/fe/api/contest/${i?"new":"edit/"+e.vp_id}`,l);switch(a.status??200){case 200:e.vp_id=a.id.toString();break;case 404:e.vp_id=(await H("/fe/api/contest/new",l)).id.toString(),i=!0;break;default:M(`Failed to modify contest ${e.vp_id} with status code ${a.status}.`)}if(await H(`/fe/api/contest/editProblem/${e.vp_id}`,'{"pids":[],"scores":{"P1000":100}}'),i){let s=await P(`/contest/edit/${e.vp_id}`);H(`/fe/api/contest/join/${e.vp_id}`,`{"code": "${s.currentData.contest.joinCode}"}`)}location.href=`https://www.luogu.com.cn/contest/${e.vp_id}`})})},`
#exlg-vp {
    margin-right: .5em;
    display: inline-block;
    flex: none;
